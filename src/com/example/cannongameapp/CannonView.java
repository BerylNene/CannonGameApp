   ...
   implements SurfaceHolder.Callback
{
   ...
   public static int LEVELS = 1; //sets the initial level as 1
   ...
   private int score =0; // sets the initial score as 0
   private static int highestScore = 0; // set the initial highest score
   Context context; 
   ...
   public CannonView(Context context, AttributeSet attrs)
   {
      ...
      highestScore = sharedPreference.getHighestScore(context);
   ...
   public void newGame()
   {
	   score = 0; // assign to score
	   LEVELS = 1; //initial level
	   TARGET_PIECES = LEVELS; //target piece increase as level increase
	   sharedPreference = new SharedPreference(); 
	   highestScore = sharedPreference.getHighestScore(context);
	   hitStates = new boolean[TARGET_PIECES];
   ...
   //nextLevel method if the player wins
   public void nextLevel()
   {
	   LEVELS = LEVELS + 1; //add 1 to the level the next level if player wins
	   TARGET_PIECES = LEVELS; // the target_piece would be the say as the level
	   sharedPreference = new SharedPreference();
	   highestScore = sharedPreference.getHighestScore(context); // players highest score
	   hitStates = new boolean[TARGET_PIECES];
	   pieceLength = (targetEnd - targetBeginning) / TARGET_PIECES;
	   for (int i = 0; i < TARGET_PIECES; i++)
	         hitStates[i] = false;
	      targetPiecesHit = 0; // when no target pieces is hit
	      blockerVelocity = initialBlockerVelocity * LEVELS; // sets initial velocity
	      targetVelocity = initialTargetVelocity * LEVELS; // sets initial velocity
	      timeLeft = 10; // start the countdown time at 10 seconds
	      cannonballOnScreen = false; //remove cannonball from the screen
	      shotsFired = 0; // set the initial number of shots fired
	      totalElapsedTime = 0.0; // set the time elapsed to zero
	      
	      //setting the starting and ending point of the blocker and the target
	      blocker.start.set(blockerDistance, blockerBeginning);
	      blocker.end.set(blockerDistance, blockerEnd);
	      target.start.set(targetDistance, targetBeginning);
	      target.end.set(targetDistance, targetEnd);
	      
	      if (gameOver) // starting a new game
	      {
	         gameOver = false; 
	         cannonThread = new CannonThread(getHolder()); // create thread
	         cannonThread.start(); // start the game loop thread
	      } 
	   } // end method nextLevel
   ...
   private void updatePositions(double elapsedTimeMS)
   {
      ...
      if (cannonballOnScreen) // if there is currently a shot fired
      {
            ...
            cannonball.y - cannonballRadius < blocker.end.y)
         {
            ...
            {
            	score = score - (15 * LEVELS);// decreases the players score by 15 times the current level
            }
         ...
         {
        	 score = score + (10 *LEVELS); // increases the players score by 10 times the players score
   ...
   public void drawGameElements(Canvas canvas)
   {
      ...
      canvas.drawText(getResources().getString(
 	         R.string.highest_score_saved, highestScore), 30, 100, textPaint);
      ...
      final DialogFragment gameResult = 
         new DialogFragment()
         {
            ...
            public Dialog onCreateDialog(Bundle bundle)
            {
               ...
               String displayMsg = getString(messageId);
               if(displayMsg.equalsIgnoreCase("Congratulations, You win!") && LEVELS <= 9){
            	 ...
            	 builder.setPositiveButton(R.string.next_level,
            		 ...
            		 // Resets game "Reset Game" Button is pressed
                     @Override
                     public void onClick(DialogInterface dialog, int which)
                     {
                        dialogIsDisplayed = false;
                        nextLevel(); // sets up and start a new game
                     } 
   	   ...
   	   if(highestScore < score){
   		   highestScore = score;
   		sharedPreference.addHighestScore(context, highestScore);
   	   }
